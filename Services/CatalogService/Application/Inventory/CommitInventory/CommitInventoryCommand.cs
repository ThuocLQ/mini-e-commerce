...
public sealed record CommitInventoryCommand(Guid OrderId) : IRequest;
...
public sealed class CommitInventoryHandler : IRequestHandler<CommitInventoryCommand>
{
    ...
    public Task Handle(CommitInventoryCommand request, CancellationToken cancellationToken) =>
        request.OrderId == Guid.Empty ? throw new ArgumentException("Order id is required.") : _repository.CommitAsync(request.OrderId, cancellationToken);
}
