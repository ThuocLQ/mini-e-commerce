...
function AddressForm({ address, busy, onCancel, onSubmit }: { address: CustomerAddress | null; busy: boolean; onCancel: () => void; onSubmit: (input: AddressInput) => void }) {
  ...
  return <form className="mt-5 border border-[var(--line)] bg-[#fbfcfa] p-4" onSubmit={submit}><div className="flex items-center justify-between gap-3"><h4 className="font-semibold">{address ? "Edit address" : "Add address"}</h4><button aria-label="Close address form" className="inline-flex h-8 items-center gap-1 text-sm text-[var(--muted)] hover:text-[var(--foreground)]" disabled={busy} onClick={onCancel} type="button"><ChevronDown aria-hidden="true" size={16} />Close</button></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><Field label="Label" maxLength={80} name="label" onChange={update} required value={input.label} /><Field label="Recipient name" maxLength={120} name="recipientName" onChange={update} required value={input.recipientName} /><Field label="Address line 1" maxLength={200} name="line1" onChange={update} required value={input.line1} /><Field label="Address line 2" maxLength={200} name="line2" onChange={update} value={input.line2 ?? ""} /><Field autoComplete="address-level2" label="City" maxLength={120} name="city" onChange={update} required value={input.city} /><CountryField onChange={update} value={input.countryCode} /><Field autoComplete="postal-code" label="Postal code" maxLength={20} name="postalCode" onChange={update} value={input.postalCode ?? ""} /></div><label className="mt-4 flex items-start gap-2 text-sm"><input checked={input.makeDefault} className="mt-0.5 size-4 accent-[var(--accent)]" onChange={(event) => update("makeDefault", event.target.checked)} type="checkbox" /><span>Make this my default delivery address</span></label><div className="mt-5 flex flex-wrap gap-2"><button className="inline-flex h-10 items-center gap-2 bg-[var(--accent)] px-4 text-sm font-semibold text-white hover:bg-[var(--accent-strong)] disabled:opacity-60" disabled={busy} type="submit">{busy ? <LoaderCircle aria-hidden="true" className="animate-spin" size={16} /> : null}{busy ? "Saving" : address ? "Save address" : "Add address"}</button><button className="h-10 border border-[var(--line)] px-4 text-sm font-semibold hover:bg-white disabled:opacity-60" disabled={busy} onClick={onCancel} type="button">Cancel</button></div></form>;
}

function CountryField({ onChange, value }: { onChange: (name: keyof AddressInput, value: string) => void; value: string }) {
  const commonCodes = ["VN", "US", "SG", "GB", "AU", "JP", "KR"];
  const normalizedValue = value.trim().toUpperCase();
  return <label className="block text-sm font-medium">Country<select autoComplete="country" className="mt-1 h-10 w-full border border-[var(--line)] bg-white px-3 font-normal outline-none focus:border-[var(--accent)]" name="countryCode" onChange={(event) => onChange("countryCode", event.target.value)} required value={normalizedValue}><option value="">Choose a country</option>{normalizedValue && !commonCodes.includes(normalizedValue) ? <option value={normalizedValue}>{normalizedValue}</option> : null}{commonCodes.map((code) => <option key={code} value={code}>{code}</option>)}</select></label>;
}
...
function Field({ label, name, onChange, required, value, ...attributes }: { label: string; name: keyof AddressInput; onChange: (name: keyof AddressInput, value: string) => void; required?: boolean; value: string } & Omit<React.InputHTMLAttributes<HTMLInputElement>, "onChange" | "value">) {
  return <label className="block text-sm font-medium">{label}<input autoComplete={name === "recipientName" ? "name" : name === "line1" ? "address-line1" : name === "line2" ? "address-line2" : undefined} className="mt-1 h-10 w-full border border-[var(--line)] bg-white px-3 font-normal outline-none focus:border-[var(--accent)]" disabled={attributes.disabled} name={name} onChange={(event) => onChange(name, event.target.value)} required={required} value={value} {...attributes} /></label>;
}
