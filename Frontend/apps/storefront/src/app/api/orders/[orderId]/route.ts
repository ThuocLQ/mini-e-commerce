...
export async function GET(request: Request, context: { params: Promise<{ orderId: string }> }) {
  ...
  try {
    const upstream = await fetch(gatewayUrl(`/orders/${encodeURIComponent(orderId)}`), {
    ...
    return NextResponse.json({ message: "Order details are unavailable. Please try again." }, { status: 503 });
  }
}
