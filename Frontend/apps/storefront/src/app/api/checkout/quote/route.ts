...
export async function POST(request: Request) {
  ...
  try {
    const upstream = await fetch(gatewayUrl("/orders/checkout/quote"), {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ message: "Order review is unavailable. Please try again." }, { status: 503 });
  }
}
...
function isCheckoutQuoteRequest(value: unknown): value is CheckoutQuoteRequest {
